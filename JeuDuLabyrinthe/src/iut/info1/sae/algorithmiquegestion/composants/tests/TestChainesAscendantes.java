...
class TestChainesAscendantes {
    ...
    /**
     * Méthode de test qui redéfinit les coordonnées et les marques initiales de
     * tout les sommets du graphe grapheTeste.
     */
    @BeforeEach
    ...
    void testMarqueDifferente() {
        // les marques ont été réinitialisées par le BeforeEach elles vallent -1 pour
        // tous les sommets
        assertFalse(grapheTeste.marqueDifferente(grapheTeste.getListeSommets()[0], grapheTeste.getListeSommets()[1]));

        grapheTeste.getListeSommets()[5].setMarque(2);
        grapheTeste.getListeSommets()[6].setMarque(5);
        assertTrue(grapheTeste.marqueDifferente(grapheTeste.getListeSommets()[5], grapheTeste.getListeSommets()[6]));
    }
    ...
    void testSommetsDeMemeMarque() {
        ...
        for (int i = 0; i < grapheTeste.sommetsDeMemeMarque(marque).size(); i++) {
            ...
            System.out.println("Sommets : " + "X : " + grapheTeste.sommetsDeMemeMarque(marque).get(i).getCoordonneeX()
                    + "  Y : " + grapheTeste.sommetsDeMemeMarque(marque).get(i).getCoordonneeY() + "\tMarques : "
                    + grapheTeste.sommetsDeMemeMarque(marque).get(i).getMarque());
        }
        ...
        Sommet[] sommetsAttendus2 = { grapheTeste.getListeSommets()[8], grapheTeste.getListeSommets()[12],
    ...
    void testDefinitUneMarque() {
        ...
        /* Test du 'if' */
        grapheTeste.definitUneMarque(grapheTeste.getListeSommets()[0], grapheTeste.getListeSommets()[1]);

        /* Test du 'if' dans le 'else if' */
        grapheTeste.getListeSommets()[6].setMarque(8);
        grapheTeste.definitUneMarque(grapheTeste.getListeSommets()[5], grapheTeste.getListeSommets()[6]);
        ...
        grapheTeste.definitUneMarque(grapheTeste.getListeSommets()[15], grapheTeste.getListeSommets()[16]);
        ...
        grapheTeste.definitUneMarque(grapheTeste.getListeSommets()[16], grapheTeste.getListeSommets()[17]);
    ...
    void testSommetAleatoires() {
        System.out.println(
                "Test de renvoie tableau contenant un sommet aléatoire " + "ainsi qu'un de ses sommets adjacent : ");
        Sommet[] lesSommets = new Sommet[2];
        lesSommets = grapheTeste.sommetsAleatoires();

        System.out.println("Sommet choisi au hasard : X = " + lesSommets[0].getCoordonneeX() + " ; Y = "
                + lesSommets[0].getCoordonneeY() + "\nl'un de ses sommet adjacent : X = "
                + lesSommets[1].getCoordonneeX() + " ; Y = " + lesSommets[1].getCoordonneeY());
    }

    /**
     * Créé un graphe à partir de l'algorithme de création de chaînes puis affiche
     * ses liaisons dans la console texte afin de vérifier sa validité
     */
    @AfterAll
