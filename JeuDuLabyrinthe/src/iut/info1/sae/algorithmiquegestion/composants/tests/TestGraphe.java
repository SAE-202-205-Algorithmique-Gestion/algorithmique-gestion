...
class TestGraphe {
    ...
    /**
     * Redéfinit les coordonnées et les marques initiales de tout les sommets du
     * graphe grapheTeste.
     */
    @BeforeEach
    ...
    /**
     * Méthode de test qui récupère les coordonnées X et Y de différents sommets
     */
    @Test
    void testRecuperationCoordonnees() {
        System.out.println("Test récupération coordonnées" + " pour un labyrinthe/graphe 5*5 :");
...
//		Graphe grapheTeste2 = new Graphe(5, 5);
    ...
    void testTousLesSommetsAdjacentsDuSommet() {
        ...
        for (int i = 0; i < grapheTeste.tousLesSommetsAdjacentsDuSommet(6).length; i++) {

            System.out.println(
                    "Sommet adjacent : X = " + grapheTeste.tousLesSommetsAdjacentsDuSommet(6)[i].getCoordonneeX()
                            + " ; Y = " + grapheTeste.tousLesSommetsAdjacentsDuSommet(6)[i].getCoordonneeY());
        }
        // Premier Test
        Sommet[] listeAttendu = { grapheTeste.getListeSommets()[11], grapheTeste.getListeSommets()[1],
                grapheTeste.getListeSommets()[7], grapheTeste.getListeSommets()[5] };

        assertEquals(listeAttendu[0].getCoordonneeX(),
                grapheTeste.tousLesSommetsAdjacentsDuSommet(6)[0].getCoordonneeX());
        assertEquals(listeAttendu[0].getCoordonneeY(),
                grapheTeste.tousLesSommetsAdjacentsDuSommet(6)[0].getCoordonneeY());
        assertEquals(listeAttendu.length, grapheTeste.tousLesSommetsAdjacentsDuSommet(6).length);

        // Second Test
        Sommet[] listeAttendu2 = { grapheTeste.getListeSommets()[19], grapheTeste.getListeSommets()[23] };

        assertEquals(listeAttendu2[0].getCoordonneeX(),
                grapheTeste.tousLesSommetsAdjacentsDuSommet(24)[0].getCoordonneeX());
        assertEquals(listeAttendu2[0].getCoordonneeY(),
                grapheTeste.tousLesSommetsAdjacentsDuSommet(24)[0].getCoordonneeY());
        assertEquals(listeAttendu2.length, grapheTeste.tousLesSommetsAdjacentsDuSommet(24).length);

        // Troisième Test
        Sommet[] listeAttendu3 = { grapheTeste.getListeSommets()[6], grapheTeste.getListeSommets()[2],
                grapheTeste.getListeSommets()[0] };

        assertEquals(listeAttendu3[2].getCoordonneeX(),
                grapheTeste.tousLesSommetsAdjacentsDuSommet(1)[2].getCoordonneeX());
        assertEquals(listeAttendu3[2].getCoordonneeY(),
                grapheTeste.tousLesSommetsAdjacentsDuSommet(1)[2].getCoordonneeY());
        assertEquals(listeAttendu3.length, grapheTeste.tousLesSommetsAdjacentsDuSommet(1).length);

        /*
         * l'assertArrayEquals fonctionne seulement si on ne met pas en commentaire
         * l'Override de equals dans la classe Sommet
         */
